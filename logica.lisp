...
(defun generar-lista-ptos-suministros ()
  ...
  (let ((query "SELECT id, nombre, latitud, longitud,costo, reconocida FROM puestos_suministros"))
...
#| (defun filtrar-puestos-procesados (puestos-procesados posicion distancia-max);Filtra los puestos de suministros que están dentro de la distancia máxima desde el punto de referencia.
        ...
        (let* ((id (first puesto))                 ;; Extraer ID
                ...
                (reconocida (sixth puesto))       ;; Extraer Longitud del puesto
           ...
           ;; (verificar-y-imprimir-tipos lat-actual long-actual lat-puesto long-puesto)
            ...
            (when (and lat-puesto long-puesto
           (not (null distancia))
  ...
  (let* ((lat-actual (first posicion)) ;; Latitud del punto de referencia
    ...
    (dolist (puesto puestos-procesados puestos-filtrados)
      (let* ((id (first puesto)) ;; Extraer ID
             ...
             (reconocida (sixth puesto)) ;; Extraer Reconocida del puesto
        ...
        (let ((distancia (calculate-distance lat-actual long-actual lat-puesto long-puesto)))
          ...
          ;; Verificar si el puesto cumple con los criterios de distancia o nombre
            (when (and (not (= distancia 0))       
                      (and lat-puesto long-puesto
                          (not (null distancia))
                          (< distancia distancia-max)
                          (null reconocida)
                      )
